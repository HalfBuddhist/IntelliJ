...
class Permutation {
    ...
    /**
     * Find the next permutation in the natural order.
     * Brute force
...
public class Permutations {
    int iterative_factorial(int p) throws Exception {
        if (p < 0)
     ...
     * generation ends when the total number counts to zero.
     * O(n*n!)
     * AC
     *
     * @param nums
    ...
    public List<List<Integer>> permute(int[] nums) {
        ...
        try {
            int cnt = iterative_factorial(nums.length);
            while (cnt-- > 0) {
